...
group2D can be run in parallel if `future.apply` is installed. The below block
demonstrates an improvement in processing time when data are grouped in
parallel.
```{r}
plan(multiprocess)
tic()
...
```{r}
plot2D(tea.data, file = 1, mod.time = 60, dead.time = 30, ion = 195.0880)
```
